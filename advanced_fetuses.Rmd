---
title: "More on Importing and Cleaning Data in the R tidyverse"
output:
  html_document:
    keep_md: yes
...
-  mutate_if and mutate_all with predicates

-  group_by and summarise

-  joining

-  customising plots - scale_colour_manual, date_format, ylim, colour wheel

-  cowplot for publishing

-  legend titles and position

-  lty pch colours


Check PDF for over-running of code lines
...
And so on.  The best approach is to copy and paste the code and just change the name of the variable each time - it is a bit tedious but it isn't difficult.  With a bit of practice, you will be surprised how quickly you can get this done (unless there are A LOT of variables).
...
### Complete code for reading and formatting the calf fetuses data:
...
fetuses <- fetuses %>%
	...
	mutate_at(vars(starts_with('tactile')), parse_factor, 
		levels=c('Visible','Not visible','Hairsack')) %>%
	mutate_at(vars(starts_with('hair'), 'eyelid'), parse_factor, levels=c('N','Y')) %>%
	mutate_at(vars(starts_with('hair'), 'eyelid'), fct_collapse, No='N', Yes='Y') %>%
	...
	mutate(sex = fct_collapse(sex, NULL='Non Diff')) %>%
	select(ID, sex, parity, parity_group, everything())
